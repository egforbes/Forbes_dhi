...
% snum = 24;
% shots(:,1) = 180710000 + snum;
% shots(:,2) = 180710000 + snum;
...
%                     of the ne profile:
                    dens_diff{k,i,j} = abs(ne_1d_left_compare{k,i,j}(1:6)-...
                        ne_1d_right_compare{k,i,j}(1:6));
                    norm_dens(k,i,j) = norm(dens_diff{k,i,j},2)/...
                        (length(dens_diff{k,i,j}(1:6)));
...
%                         (length(dens_diff{k,i,j}));
    ...
    if quit2nextShot == 0 % for debugging
        ...
        h11 = pcolor(x_twin_red_adj,y_twin_red_adj,flipud(den_int_full));
        ...
        set(gca,'xtick',[0.135 0.14 0.145]);
