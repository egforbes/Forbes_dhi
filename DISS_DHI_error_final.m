...
clear all; clc;
close all;
directory_save = 'M:\M_drive\Users\Eleanor_Forbes\Holography\holo_error';
...
% Plotting parameters:
% cross_sect = [12 48 84];
cross_sect = round(321*[0.25 0.5 0.85]);%round(75*(0.5));%round(75*[0.25 0.5 0.75]); %
z_loc = 0.08;
...
step_sm = 3.85e-6;%6e-6; %step_sm+2e-6;
d_fringe = 20e-6; %d_fringe + 10e-6;
% d =15*2.54/100;
...
for shot_ind = 1:size(shots,1)
    ...
    shotnum_def = shots(shot_ind,1);
        ...
        %% Reconstruct the experimental hologram, generate the 2-D line-integrated density map:
        ...
        % note that the sign of den_int_full here depends on the optical set up
        % (e.g. the twin images can flip if the scene and reference beams flip
        % orientation).
        sign_twin = -1;
        ...
        num_of_centroids = 10;%10;%25;
        
        % identify number of n_edge values:
        num_of_n_edge = 20;%25;
        n_edge_delta = 5e20;%5e22;%5e21;
        
        %% Identify the optimal centroid and edge density for the Abel inversion for each impact parameter:
        ...
        for k = 1:num_of_cross_sect
            ...
            % display warning indicating centroid search exceeds bounds of
            % image:
            if centroid_ind_tmp <= 0 || centroid_ind_tmp+2*num_of_centroids > size(unwrapped,1)
                narrow_centroid_search_reqd(shot_ind) = 1;
                quit2nextShot = 1;
                break;
            else
                narrow_centroid_search_reqd(shot_ind) = 0;
                quit2nextShot = 0;
            end
            ...
            for i = 1:2*num_of_centroids
                ...
                for j = 1:2*num_of_n_edge
                    ...
                    % computing the L2 norm of the difference in the sides of the ne profile:
                          dens_diff{k,i,j} = abs(ne_1d_left_compare{k,i,j}(1:2)-...
                        ne_1d_right_compare{k,i,j}(1:2));
                    norm_dens(k,i,j) = norm(dens_diff{k,i,j},2)/...
                        (length(dens_diff{k,i,j}(1:2)));
...
%                     norm_dens(k,i,j) = norm(dens_diff{k,i,j},2)/(length(dens_diff{k,i,j})); %(1:length(dens_diff{k,i,j}))
    ...
    %%    
        if quit2nextShot == 0
            ...
            %% Forming the den_num_full matrix:
...
%                     end
                ...
                %% Interpolating synthetically reconstructed density and computing error:
                for k = 1:length(cross_sect)
...
%             line([cross_sect(1)*del_x+x_0 cross_sect(1)*del_x+x_0],[y_twin_red_adj(1) y_twin_red_adj(end)],'color','k');
            line([cross_sect(1)*del_x+x_0 cross_sect(1)*del_x+x_0],[y_twin_red_adj(1) y_twin_red_adj(end)],'color','k','linewidth',3);
            line([cross_sect(1)*del_x+x_0 cross_sect(1)*del_x+x_0],[y_twin_red_adj(1) .001608],'color','r','linewidth',3);
            %             if length(cross_sect)==3
                line([cross_sect(2)*del_x+x_0 cross_sect(2)*del_x+x_0],[y_twin_red_adj(1) y_twin_red_adj(end)],'color','k','linewidth',3);%,'linestyle',':');
                line([cross_sect(2)*del_x+x_0 cross_sect(2)*del_x+x_0],[y_twin_red_adj(1) .001952],'color','r','linewidth',3);

                line([cross_sect(3)*del_x+x_0 cross_sect(3)*del_x+x_0],[y_twin_red_adj(1) y_twin_red_adj(end)],'color','k','linewidth',3);%,'linestyle','--');
                line([cross_sect(3)*del_x+x_0 cross_sect(3)*del_x+x_0],[y_twin_red_adj(1) .002527],'color','r','linewidth',3);
...
save(['M:\Users\Michael_Ross\Coding\Holography_Reconstruction\dhi_error_data_',...
    num2str(shotnum_def),'.mat'],...
    'rad_l','rad_r','den_num_axial_l_plt','den_num_axial_r_plt','n_error_out_interp_l',...
    'n_error_out_interp_r');
...
% writing matrices to text files to send to Uri:
csvwrite('M:\Users\Michael_Ross\Conferences\APS2016\phase_unwrapped.txt',unwrapped);
csvwrite('M:\Users\Michael_Ross\Conferences\APS2016\dens_integrated.txt',den_int_full);
csvwrite('M:\Users\Michael_Ross\Conferences\APS2016\dens_number.txt',den_num_full_axial);
csvwrite('M:\Users\Michael_Ross\Conferences\APS2016\z_axis.txt',x_twin_red_adj);
csvwrite('M:\Users\Michael_Ross\Conferences\APS2016\y_axis.txt',y_twin_red_adj);

unwrapped = csvread('M:\Users\Michael_Ross\Conferences\APS2016\phase_unwrapped.txt')
den_int_full = csvread('M:\Users\Michael_Ross\Conferences\APS2016\dens_integrated.txt')
den_num_full_axial = csvread('M:\Users\Michael_Ross\Conferences\APS2016\dens_number.txt')
x_axis = csvread('M:\Users\Michael_Ross\Conferences\APS2016\z_axis.txt');
y_axis = csvread('M:\Users\Michael_Ross\Conferences\APS2016\y_axis.txt');

figure(1); imagesc(x_axis,y_axis,unwrapped);
set(gca,'ydir','normal');
figure(2); imagesc(x_axis,y_axis,den_int_full);
figure(3); imagesc(x_axis,y_axis,den_num_full_axial);
...
col_sec = 'g';
subplot(3,1,1);hold on;
p1 = plot(rad_l{1},den_num_axial_l{cross_sect(1)},col_sec,'linewidth',3);
p2 = plot(rad_r{1},den_num_axial_r{cross_sect(1)},'k','linewidth',3);
% shadedErrorBar(rad_r{1},n_prof_r{1},n_error_r{1},col_sec,1);
% shadedErrorBar(rad_r{1},n_prof_r{1},n_error_r{1},'k',1);
h40011 = errorbar(rad_l{1},n_prof_l{1},n_error_l{1},'color',col_sec);
h40012 = errorbar(rad_r{1},n_prof_r{1},n_error_r{1},'color','k');
...
if length(cross_sect) ==3
    subplot(3,1,2);hold on;
    plot(rad_l{2},den_num_axial_l{cross_sect(2)},col_sec,'linewidth',3);
    plot(rad_r{2},den_num_axial_r{cross_sect(2)},'k','linewidth',3);
%     shadedErrorBar(rad_r{2},n_prof_r{2},n_error_r{2},col_sec,1);
%     shadedErrorBar(rad_r{2},n_prof_r{2},n_error_r{2},'k',1);
    h40021 = errorbar(rad_l{2},n_prof_l{2},n_error_l{2},'color',col_sec);
    errorbar(rad_r{2},n_prof_r{2},n_error_r{2},'color','k');
    ...
    subplot(3,1,3);hold on;
    plot(rad_l{3},den_num_axial_l{cross_sect(3)},col_sec,'linewidth',3);
    plot(rad_r{3},den_num_axial_r{cross_sect(3)},'k','linewidth',3);
%     shadedErrorBar(rad_l{3},n_prof_l{3},n_error_l{3},col_sec,1);
%     shadedErrorBar(rad_r{3},n_prof_r{3},n_error_r{3},'k',1);
    errorbar(rad_l{3},n_prof_l{3},n_error_l{3},'color',col_sec);
    errorbar(rad_r{3},n_prof_r{3},n_error_r{3},'color','k');
...
% scatter(x_twin_red_adj,y_twin_red_adj(centroid_abs),'k','.');


figure(5);
waterfall(den_num_full_axial);
figure(6);
plot(den_num_full_axial(1,:));
...
plot(den_num_l{1},'color','k','linewidth',2);
plot(den_num_axial_l{1},'color','m');
subplot(2,3,2); hold on;
plot(-(den_num_l{1}-den_num_axial_l{1}),'color','g');
subplot(2,3,3); hold on;
plot(den_int_edge_axial./x_edge_r);
plot(den_int_edge_axial,'color','m');
subplot(2,3,4); hold on;
plot(den_num_r{1},'color','k','linewidth',2);
plot(den_num_axial_r{1},'color','m');
subplot(2,3,5); hold on;
plot(-(den_num_r{1}-den_num_axial_r{1}),'color','g');
...
line([cross_sect(1)*del_x+x_0 cross_sect(1)*del_x+x_0],[y_twin_red_adj(1) y_twin_red_adj(end)],'color','k');
if length(cross_sect)==3
    line([cross_sect(2)*del_x+x_0 cross_sect(2)*del_x+x_0],[y_twin_red_adj(1) y_twin_red_adj(end)],'color','k','linestyle',':');
    line([cross_sect(3)*del_x+x_0 cross_sect(3)*del_x+x_0],[y_twin_red_adj(1) y_twin_red_adj(end)],'color','k','linestyle','--');
else
end
title(['def = ',num2str(shotnum_def),'; base = ',num2str(shotnum_base)]);
%%% Notes:  can probably delete after code up and running:
...
phase_x_adj = linspace(-max(phase_x)/2,max(phase_x)/2,length(phase_x));
phase_y_adj = linspace(-max(phase_y)/2,max(phase_y)/2,length(phase_y));
h_115 = imagesc(phase_x_adj,phase_y_adj,phase_diff);axis image; colormap gray; set(gca,'Ydir','normal');%shading interp;
% set(h_115,'edgecolor','none');
colorbar;
set(gca,'xtick',[-0.02:0.01:0.02]);
set(gca,'ytick',[-0.02:0.01:0.02]);

title('Reconstructed Phase [rad]');
...
% phase_x_adj = linspace(-max(phase_x)/2,max(phase_x)/2,length(phase_x));
% phase_y_adj = linspace(-max(phase_y)/2,max(phase_y)/2,length(phase_y));
h_115 = imagesc(phase_x,phase_y,phase_diff);axis image; colormap gray; set(gca,'Ydir','normal');%shading interp;
% set(h_115,'edgecolor','none');
colorbar;
% set(gca,'xtick',[-0.02:0.01:0.02]);
% set(gca,'ytick',[-0.02:0.01:0.02]);
...
set(gca,'fontsize',fnt);
        ...
        figure(301);
        subplot(2,1,1); hold on; grid on;
        h1 = plot(rad_l{k},n_prof_l{k},'color','k');
        h2 = plot(rad_rec_l{k},den_num_rec_l{k},'color','m');
        leg301 = legend([h1 h2],'n_prof_l{k}','den_num_rec_l_plt{k}');
        set(leg301,'Interpreter','none');
        
        
        figure(302);
        
        subplot(2,1,1); hold on; grid on;
        h1 = plot(n_prof_l{k},'color','k','linewidth',2);
        h2 = plot(den_num_rec_l_plt{k}','color','m');
        % h3 = plot(den_num_rec_l_plt{k}'/2,'color','g');
        title('left');
        subplot(2,1,2); hold on; grid on;
        % plot(n_prof_r{k},'color','k','linewidth',2);
        plot(den_num_rec_r_plt{k}','color','m');
        title('right');
        leg302 = legend([h1 h2],'n_prof_l{k}','den_num_rec_l_plt{k}');
        set(leg302,'Interpreter','none');
        
        figure(303);
        
        subplot(2,1,1);hold on;
        h1 = plot(Ne_1d_left{cross_sect(k),centroid(cross_sect(k))}-min(Ne_1d_left{cross_sect(k),centroid(cross_sect(k))}),'color','k','linewidth',2);
        h2 = plot(N_prof_l{k}-min(N_prof_l{k}),'color','m');
        title('left');
        subplot(2,1,2);hold on;
        plot(Ne_1d_right{cross_sect(k),centroid(cross_sect(k))}-min(Ne_1d_right{cross_sect(k),centroid(cross_sect(k))}),'color','k','linewidth',2);
        plot(N_prof_r{k},'color','m');
        title('right');
        leg303 = legend([h1 h2],'Ne_1d_left{cross_sect(1),centroid(cross_sect(1))}','N_prof_l{1}');
        set(leg303,'Interpreter','none');
        
        
        
        
        % Plotting linear density versus axial direction:
        
        fnt = 18;
        figure(500);
        set(gca,'position',[0.1300 0.4096 0.7100 0.5154]);
...
%         fig1 = figure(1);hold on;
        ...
        cbar1 = colorbar;
        shift = 0.10;
        set(cbar1,'position',[0.8104+shift 0.4095 0.0476-.02 0.5143]);
        xlim([x_twin_red_adj(1) x_twin_red_adj(end)]);
        ylim([y_twin_red_adj(1) y_twin_red_adj(end)]);
        % line([cross_sect(1)*del_x+x_0 cross_sect(1)*del_x+x_0],[y_twin_red_adj(1) y_twin_red_adj(end)],'color','k','linewidth',3);
        % if length(cross_sect)==3
...
%         xlabel('Axial distance [m]','fontsize',fnt);
        ...
        subplot(3,1,3); hold on;
        p500_1 = plot(x_twin_red_adj,N_linear_density_l,'color','r','linewidth',4);
        p500_2 = plot(x_twin_red_adj,N_linear_density_r,'color','k','linewidth',4);
        xlim([x_twin_red_adj(1) x_twin_red_adj(end)]);
        ylim([3e18 11e18]);
        xlabel('Axial distance [m]');
        ylabel('Linear density, N [m^{-1}]');
        set(gca,'xtick',[0.145 0.15 0.155]);
        leg1 = legend([p500_1 p500_2],'bottom','top');
        set(gca,'fontsize',fnt);
